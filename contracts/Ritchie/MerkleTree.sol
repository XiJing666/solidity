...
library MerkleProof {
    ...
    // 利用proof数来验证leaf是否属于根为root的MekleTree中，如果是，则返回true，它调用了processProof函数。
    function verify(
    ...
    // 利用proof和leaf依次计算出Merkle Tree的root，它调用了_hashPair()函数
    ...
    // Sorted Pair Hash
    // 用keccak256()函数计算非根节点对应的两个子节点的hash
...
contract MerkleTree is ERC721{
    bytes32 immutable public root;// Merkle数的根
    mapping(address => bool) public mintedAddress; //记录已经mint的地址

    // 构造函数，初始化NFT合集的名称，代号，Merkle数的根
    constructor(string memory name, string memory symbol, bytes32 merkleroot) ERC721(name, symbol) {
        root = merkleroot;
    }

    // 利用Merkle数来验证地址并且mint
    function mint(address account,uint256 tokenId,bytes32[] calldata proof) external{
        require(_verify(_leaf(account),proof),'error');// Merkle验证通过
        require(!mintedAddress[account],'error');// 地址没有mint过
