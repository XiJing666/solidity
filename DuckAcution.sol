...
contract DucthAuction is Ownable,ERC721 {
    ...
    // 设定拍卖启始时间：在构造函数中声明当前区块时间为启始时间，项目方也可以通过`setAuctionStartTime(uint32)`函数来调整
    ...
    function auctionMint(uint256 quantity) external payable{
        ...
        require(_saleStartTime != 0 && block.timestamp >= _saleStartTime,'error');// 检查是否设置起拍时间以及拍卖是否开始
        require(totalSupply()  + quantity <= COLLECTOIN_SIZE,'error');//检查是否超过NFT上限

        uint256 totalCost = getAuctionPrice() * quantity; // 计算mint成本
        require(msg.value >= totalCost,'error'); // 检查是否支付足够ETH
        ...
        if(msg.value > totalCost) {
            payable(msg,sender).transfer(msg.value - totalCost); //可能会有重入风险
        }
    }
